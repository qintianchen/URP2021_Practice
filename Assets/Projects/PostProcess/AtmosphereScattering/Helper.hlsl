...
bool SphereRayIntersectTest(float3 spherePosition, float radius, float3 rayPosition, float3 rayDir)
{
    rayDir = normalize(rayDir);
    
    float3 rayToSphere = spherePosition - rayPosition;
    
    float a2 = dot(rayToSphere, rayToSphere);
    float r2 = radius * radius;
    if(a2 <= r2) return true; // 射线原点在球内部

    float l = dot(rayToSphere, rayDir);
    if(l < 0) return false; // 射线是远离球体
