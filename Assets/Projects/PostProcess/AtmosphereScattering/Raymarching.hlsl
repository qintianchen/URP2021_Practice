...
{
    if(SphereRayIntersectTest(param.PlanetRadius, float3(0, 0, 0), p, dir))
    {
        return 0;
    }
...
// 积分计算多重散射查找表
float3 IntegralMultiScattering(in AtmosphereParameter param, float3 samplePoint, float3 lightDir, Texture2D _transmittanceLut, SamplerState samplerLinearClamp)
{
...
// 计算天空颜色
float3 GetSkyView(in AtmosphereParameter param, float3 eyePos, float3 viewDir, float3 lightDir, float maxDis, Texture2D _transmittanceLut, Texture2D _multiScatteringLut, SamplerState samplerLinearClamp)
{
    const int N_SAMPLE = 32;
    float3 color = float3(0, 0, 0);

    // 光线和大气层, 星球求交
    float dis = RayIntersectSphere(float3(0,0,0), param.PlanetRadius + param.AtmosphereHeight, eyePos, viewDir); // 视线与大气层的交点
    float d = RayIntersectSphere(float3(0,0,0), param.PlanetRadius, eyePos, viewDir); // 视线与星球的交点
    if(dis < 0) return color; // 视线与大气层没有交点（一般这个不会发生）
    if(d > 0) dis = min(dis, d); // 视线与星球有交点，则光线步进只步进到星球交点
